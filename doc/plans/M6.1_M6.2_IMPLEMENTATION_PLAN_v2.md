...
- **Steps 1-3:** Agent startup and registration (LM → REF → Players)
...
Error Codes:
  E001-E018 - See ../reference/error_codes_reference.md
...
Documentation:
  ...
  System Integration: system_integration_verification_plan.md
        """,
    )
...
**File:** `../reference/error_codes_reference.md`
...
**Documentation:** system_integration_verification_plan.md#5-match-execution
...
**Documentation:** system_integration_verification_plan.md#10-auth-token-verification
...
**Documentation:** system_integration_verification_plan.md#2-launch-referees
...
**Documentation:** system_integration_verification_plan.md#13-common-failure-points
...
**Documentation:** system_integration_verification_plan.md#4-start-league-orchestration
...
```
http://localhost:8000/mcp       ✓ Valid
http://0.0.0.0:8101/mcp         ✓ Valid
https://api.example.com/mcp     ✓ Valid (if using HTTPS)
localhost:8000                  ✗ Invalid (missing protocol)
http://localhost/mcp            ✗ Invalid (missing port)
```
...
- **System Integration Plan:** system_integration_verification_plan.md
...
**Self-Verify:**
```bash
ls -lh ../reference/error_codes_reference.md
wc -l ../reference/error_codes_reference.md  # Should be ~600-700 lines
grep "^## E0" ../reference/error_codes_reference.md | wc -l  # Should be 18
```
...
show_error() {
    ...
    case "$error_code" in
        E001)
            ...
            echo ""
            echo "Documentation: ../reference/error_codes_reference.md#e001-timeout_error"
            ;;
        E002)
            echo "Suggested actions:"
            ...
            echo ""
            echo "Documentation: ../reference/error_codes_reference.md#e002-invalid_message_format"
            ;;
        E004)
            echo "Suggested actions:"
            ...
            echo ""
            echo "Documentation: ../reference/error_codes_reference.md#e004-agent_not_registered"
            ;;
        E008)
            echo "Suggested actions:"
            ...
            echo ""
            echo "Documentation: ../reference/error_codes_reference.md#e008-league_not_found"
            ;;
        E016)
            echo "Suggested actions:"
            ...
            echo ""
            echo "Documentation: ../reference/error_codes_reference.md#e016-service_unavailable"
            ;;
        E018)
            echo "Suggested actions:"
            ...
            echo ""
            echo "Documentation: ../reference/error_codes_reference.md#e018-invalid_endpoint"
            ;;
        *)
            echo "See ../reference/error_codes_reference.md for recovery suggestions"
            ;;
    esac
}
...
for SCRIPT in scripts/*.sh; do
    if [ -x "$SCRIPT" ]; then
        ...
        "$SCRIPT" --help | grep -q "\--plain" || {
            echo "❌ FAIL: $SCRIPT doesn't support --plain"
            exit 1
        }
    fi
done
echo "✅ All scripts support --plain"

# Test 2: Plain output has no emojis
echo ""
echo "Test 2: Plain output emoji-free..."
OUTPUT=$(./scripts/check_health.sh --plain 2>&1 || true)
if echo "$OUTPUT" | grep -q "[✅❌⚠️🎮⚖️📋🔍🚀]"; then
    echo "❌ FAIL: Plain output contains emojis"
    exit 1
fi
echo "✅ Plain output is emoji-free"

# Test 3: All agents support --plain
echo ""
echo "Test 3: Agent --plain support..."
python -m agents.player_P01.main --help | grep -q "\--plain" || {
    echo "❌ FAIL: Player agents don't support --plain"
    exit 1
}
echo "✅ All agents support --plain"
...
for AGENT in league_manager player_P01 player_P02 player_P03 player_P04 referee_REF01 referee_REF02; do
    ...
    python -m agents.${AGENT}.main --help > /dev/null || {
        echo "❌ FAIL: $AGENT --help failed"
        exit 1
    }
done
echo "✅ All agents support --help"
...
for AGENT in league_manager player_P01 referee_REF01; do
    ...
    python -m agents.${AGENT}.main --version 2>&1 | grep -q "1.0.0" || {
        echo "❌ FAIL: $AGENT --version failed"
        exit 1
    }
done
echo "✅ All agents support --version"
...
python -m agents.player_P01.main --invalid-arg 2>&1 | grep -q "error: unrecognized arguments" || {
    echo "❌ FAIL: Invalid args not rejected"
    exit 1
}
echo "✅ Invalid arguments rejected correctly"
...
python -m agents.player_P01.main --verbose --quiet 2>&1 | grep -q "mutually exclusive" || {
    echo "❌ FAIL: --verbose and --quiet should be mutually exclusive"
    exit 1
}
echo "✅ Mutually exclusive args validated"
...
python -m agents.player_P01.main --port 100 2>&1 | grep -q "must be between 1024-65535" || {
    echo "❌ FAIL: Invalid port not rejected"
    exit 1
}
echo "✅ Port validation works"
...
See `../reference/error_codes_reference.md` for detailed error code documentation.
...
```markdown
### M6.1: CLI Argument Parsing ✅ COMPLETED (2025-12-27)
**Priority:** P1 (High)
...
### M6.2: Operational Scripts ✅ COMPLETED (2025-12-27)
**Priority:** P1 (High)
...
### M6.6: Usability Analysis ✅ COMPLETED (2025-12-27)
**Priority:** P1 (High)
...
- [x] ../reference/error_codes_reference.md created with all 18 error codes
...
**Files Created:**
- ../reference/error_codes_reference.md (600+ lines, comprehensive error documentation)
...
- [ ] ../reference/error_codes_reference.md created with all 18 error codes
...
- [ ] **Usability:** Nielsen's heuristics score ≥7/10 for each heuristic
...
rm ../reference/error_codes_reference.md
...
1. `../reference/error_codes_reference.md` - Complete error code documentation (~600 lines)
...
**Compliance:** M6.1 ✅, M6.2 ✅, M6.6 ✅
**Accessibility:** WCAG 2.1 Level AA Compliant
**Usability:** Nielsen's Heuristics ≥7/10 each
