...
def parse_args():
    """
    Parse command-line arguments for Referee REF02.
    ...
    default_referee_id = _default_referee_id(agents_config, "REF02")
    ...
    parser = argparse.ArgumentParser(
        prog="referee_REF02",
        description="Referee Agent REF02 for Even/Odd League Multi-Agent System",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Show help
  python -m agents.referee_REF02.main --help

  # Start with custom referee ID and port
  python -m agents.referee_REF02.main --referee-id REF06 --port 8006

  # Start in screen reader mode (accessibility)
  python -m agents.referee_REF02.main --plain

  # Start with verbose logging
  python -m agents.referee_REF02.main --verbose --log-level DEBUG

  # Start with custom league
  python -m agents.referee_REF02.main --league-id league_2025_custom
...
""",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="Referee Agent REF02 1.0.0 (league.v2 protocol)",
    )
    ...
    if not (1024 <= args.port <= 65535):
        parser.error(
            ...
            f"  Suggested fix: Use --port <valid_port> (e.g., --port 8002)"
        )
