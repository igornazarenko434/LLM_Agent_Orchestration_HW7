"""
Entry point for Player Agent P02.
...
def parse_args():
    """
    Parse command-line arguments for Player P02.
    ...
    player_record = next(
        (p for p in agents_config.get("players", []) if p.get("agent_id") == "P02"),
        {},
    )

    parser = argparse.ArgumentParser(
        prog="player_P02",
        description="Player Agent P02 for Even/Odd League Multi-Agent System",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Start with defaults from config
  python -m agents.player_P02.main

  # Override player ID and port
  python -m agents.player_P02.main --player-id P05 --port 8105

  # Enable debug logging
  python -m agents.player_P02.main --log-level DEBUG

  # Plain output for screen readers (no emojis)
  python -m agents.player_P02.main --plain

  # Verbose mode for troubleshooting
  python -m agents.player_P02.main --verbose

  # JSON output for automation/CI
  python -m agents.player_P02.main --json
...
Documentation:
  ...
  System Integration: doc/plans/system_integration_verification_plan.md
    ...
    default_port = (
        player_record.get(
            "port", system_config.network.player_port_start + 1 if system_config else 8102
        )
        if player_record
        else 8102
    )
    ...
    parser.add_argument(
        "--player-id",
        type=str,
        default=os.getenv("AGENT_ID") or player_record.get("agent_id", "P02"),
        metavar="ID",
        help="Player ID (default: P02 from config or env AGENT_ID)",
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="Player Agent P02 1.0.0 (league.v2 protocol)",
    )
